...
router.get("/new", function (request, response, next) {
    response.render("books/add_book", {
        layout: "books_layout"
    });
});
...
router.get("/:id", function (request, response, next) {
    databaseConnection("books")
        ...
        .then(function (books) {
            ...
            response.render("books/get_book", {
                layout: "books_layout",
                book: books[0]
            });
        });
...
router.get("/", function (request, response, next) {
    databaseConnection("books")
        ...
        .then(function (records) {
            ...
            response.render("books/list_books", {
                layout: "books_layout",
                books: books
            });
        });
...
router.get("/delete/:id", function (request, response, next) {
    databaseConnection("books")
        ...
        .then(function (books) {
            var books = mapAuthorsToBooks(books);
            response.render("books/delete_book", {
                layout: "books_layout",
                book: books[0]
            });
        });
...
router.get("/edit/:id", function (request, response, next) {
    Promise.all([
        ...
        response.render("books/edit_book", {
            layout: "books_layout",
            book: books[0],
            authors: results[1]
        });
...
router.post("/", function (request, response, next) {
    request.checkBody("title", "Title is empty or too long").notEmpty().isLength({
        max: 255
    });
    request.checkBody("genre", "Genre is empty or too long").notEmpty().isLength({
        max: 255
    });
    request.checkBody("description", "Description is missing or too long").notEmpty().isLength({
        max: 2000
    });
